...
Partial Class PF
    ...
    End Sub
    Public Sub BindData()
        'jctdev3..JCT_Payroll_Non_Refundable_PFLoan c
        ...
        Dim SqlPass = "SELECT  a.* ,b.* FROM    jctdev3..JCT_Payroll_PeriodWise_PF_EPS_Contribution a LEFT OUTER JOIN jctdev3..spintt b ON a.EmployeeCode = b.empcd WHERE   PreviousEmployeeCode= '" + gencode() + "' "
        Dim Dr As SqlDataReader = Obj.FetchReader(SqlPass)
        Try
                ...
                While Dr.Read()
                    ...
                    If Dr.Item("PFAPR") > 0 Then
                        ...
                        If IIf(Dr.Item("Month") Is System.DBNull.Value, 0, Dr.Item("Month")) = "04" Or IIf(Dr.Item("Month") Is System.DBNull.Value, 0, Dr.Item("Month")) = "4" Then
                            ...
                            'Ten persons are sitting in two parallel rows in such a way that five of them sitting in each row with equal distance from each other. 
                            'The person in each row is facing the person in the other row. In row-1, A, B, C, D and E are sitting and facing south. In row-2, P, Q, R, S and T are sitting 
                            'and facing north. Each of them working in different banks viz., IOB, RBL, BOB, TMB, UBI, DBS, CUB, SBI, PNB and KMB but not necessary in the same order.
                            'The one who is working in CUB faces the person who sits third to the right of S. Only one person sits between C and the one who is working in CUB. 
                            ' The one who is working in BOB faces the person who sits second to the left of R. B sits immediate right of the one who is working in BOB. 
                            ' Only one person sits between R and the one who is working in IOB. Two persons are sitting between Q and the one who is working in TMB, who does not sit any of the 
                            ' extreme ends of the row. D is an immediate neighbour of the one who faces person who is working in TMB. As many persons sitting to the left of T is same as sitting
                            ' to the left of the one who is working in DBS, who does not sit opposite to T. T is not working in DBS. The one who is working in RBL sits second to the left of E.
                            '  The one who is working in KMB faces the person who sits second to the right of the person who is working in PNB. The one who is working in UBI and KMB does not 
                            ' sit any of the extreme ends of the rows.
    ...
    Public Sub IntRate()
        ...
        Dim SqlPass = "select Rate From jctdev3..Jct_Empg_Rate where CurrentYear= '" & CurYear & "' "
        Dim Dr As SqlDataReader = Obj.FetchReader(SqlPass)
        Try
            If Dr.HasRows = True Then
                While Dr.Read()
    ...
    Public Sub CheckLoan()
        ...
        'CurYear = "2007-2008"
        Dim SqlPass = "Select SanctionNo,SanctionDate,OwnLoanAmt,EmpLoanAmt,OwnIntAmt,EmpIntAmt,VPFAmt,VPFInt,Month(SanctionDate) as LoanMonth,Year(SanctionDate) as LoanYear  from jctdev3..JCT_Payroll_Non_Refundable_PFLoan where PreviousEmployeeCode='" + gencode() + "' "
        Dim Dr As SqlDataReader = Obj.FetchReader(SqlPass)
        Try
            If Dr.HasRows = True Then
                While Dr.Read()
    ...
    Public Sub OpeBal()
        ...
        Dim SqlPass = "select * from jctdev3..JCT_Payroll_PeriodWise_PF_Opening_Balance where  PreviousEmployeeCode ='" + gencode() + "' "
        Dim Dr As SqlDataReader = Obj.FetchReader(SqlPass)
        Try
    ...
    Private Function gencode() As String
        Try
            ...
            Dim cmd As SqlCommand = New SqlCommand("SELECT DISTINCT TOP(1) EmployeeCode from dbo.JCT_payroll_employees_master where  newEmployeeCode ='" + Session("Empcode") + "' AND STATUS= 'A' AND Active = 'Y'", Obj.Connection())
