...
Partial Class PF
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not (Page.IsPostBack) Then
            ...
            'Added 30 OCT 2019 For Extention 
            Dim SqlPass = "SELECT 'X' FROM JCT_Payroll_PeriodWise_PF_Opening_Balance WHERE EmployeeCode= '" + Session("Empcode") + "' AND FYPeriod = '2020-21' AND status	='A' AND DtPfTaken is    NULL "
            Dim Dr As SqlDataReader = Obj.FetchReader(SqlPass)
            Try

                If Dr.HasRows = False Then
                    'While Dr.Read()
                    Dim script As String = "alert('Not Entitled Because You Have Taken PF Already');"
    ...
    Public Sub BindData()
        ...
        'Dim SqlPass = "SELECT  a.* ,b.* FROM    JCTDEV..JCT_Payroll_PeriodWise_PF_EPS_Contribution a LEFT OUTER JOIN JCTDEV..spintt b ON a.EmployeeCode = b.empcd WHERE   PreviousEmployeeCode= '" + gencode() + "' and fyperiod = '2020-21' and a.status = 'A' "

        Dim qrstr As String = ConfigurationManager.ConnectionStrings("misjctdev").ToString()
        Dim mycon As SqlConnection = New SqlConnection(qrstr)
        Dim SqlPass As String = "Jct_Payroll_PortalPF_BindData"
        Dim cmd1 As SqlCommand = New SqlCommand(SqlPass, mycon)
        cmd1.CommandType = CommandType.StoredProcedure
        cmd1.Parameters.Add("@Empcode", SqlDbType.VarChar, 10).Value = gencode()
        mycon.Open()
    ...
    Public Sub IntRate()
        ...
        Dim SqlPass = "select isnull(Rate,0) as Rate From Jctdev..Jct_Empg_Rate where CurrentYear= '" & CurYear & "' "
        Dim Dr As SqlDataReader = Obj.FetchReader(SqlPass)
        Try
            If Dr.HasRows = True Then
                While Dr.Read()
    ...
    Public Sub CheckLoan()
        ...
        'CurYear = "2007-2008"
        Dim SqlPass = "Select SanctionNo,SanctionDate,isnull(OwnLoanAmt,0) as OwnLoanAmt ,isnull(EmpLoanAmt,0) as EmpLoanAmt,isnull(OwnIntAmt,0) as OwnIntAmt,isnull(EmpIntAmt,0) as EmpIntAmt,isnull(VPFAmt,0) as VPFAmt,isnull(VPFInt,0) as VPFInt,Month(SanctionDate) as LoanMonth,Year(SanctionDate) as LoanYear  from JCTDEV..JCT_Payroll_Non_Refundable_PFLoan where PreviousEmployeeCode='" + gencode() + "' "
        Dim Dr As SqlDataReader = Obj.FetchReader(SqlPass)
        Try
            If Dr.HasRows = True Then
                While Dr.Read()
    ...
    Public Sub OpeBal()
        'JCTDEV..JCT_Payroll_Non_Refundable_PFLoan c
        'Dim SqlPass = "select * from JCTDEV..JCT_Payroll_PeriodWise_PF_Opening_Balance where  PreviousEmployeeCode ='" + gencode() + "'  and status =	 'a'  and fyperiod = '2020-21' "
        'Dim Dr As SqlDataReader = Obj.FetchReader(SqlPass)

        Dim qrstr As String = ConfigurationManager.ConnectionStrings("misjctdev").ToString()
        Dim mycon As SqlConnection = New SqlConnection(qrstr)
        Dim SqlPass As String = "Jct_Payroll_PortalPFOpenBal_BindData"
        Dim cmd1 As SqlCommand = New SqlCommand(SqlPass, mycon)
        cmd1.CommandType = CommandType.StoredProcedure
        cmd1.Parameters.Add("@Empcode", SqlDbType.VarChar, 10).Value = gencode()
        mycon.Open()
